...
ensm_hyperpar <- function(C, X, A_block, L, max_iter, 
	...
	## finding starting hyper-parameters:
	n_tr <- dim(X)[1]
	Y <- ihs_transform(C)
	...
	om <- 0 # should not matter because opt_Y = 4
	oy <- 4
	
	l <- 1

	Rcpp::sourceCpp('src/main.cpp')
	tmp <- ensm_cluster_mean(C, X, A_block, L = l, max_iter_input = max_iter, 
	...
	tmp_new <- ensm_cluster_mean(C, X, A_block, L = L, max_iter_input = max_iter, 
             ...
             update_sigma2_ = FALSE, starting_sigma2_ = tmp$sigma2_laplace)

	final_hyperpar <- c(a1_new, a2, b1_new, b2, alpha_sigma, beta_sigma, tmp$sigma2_laplace)
